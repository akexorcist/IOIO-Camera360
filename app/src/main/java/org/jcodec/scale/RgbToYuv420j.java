    ...
    public void transform(Picture img, Picture dst) {
        ...
        for (int i = 0; i < img.getHeight() >> 1; i++) {
            for (int j = 0; j < img.getWidth() >> 1; j++) {
                ...
                rgb2yuv((byte) y[offSrc], (byte) y[offSrc + 1], (byte) y[offSrc + 2], out[0]);
                dstData[0][offLuma] = (byte) out[0][0];

                rgb2yuv((byte) y[offSrc + strideSrc], (byte) y[offSrc + strideSrc + 1], (byte) y[offSrc + strideSrc + 2], out[1]);
                dstData[0][offLuma + strideDst] = (byte) out[1][0];

                ++offLuma;

                rgb2yuv((byte) y[offSrc + 3], (byte) y[offSrc + 4], (byte) y[offSrc + 5], out[2]);
                dstData[0][offLuma] = (byte) out[2][0];

                rgb2yuv((byte) y[offSrc + strideSrc + 3], (byte) y[offSrc + strideSrc + 4], (byte) y[offSrc + strideSrc + 5], out[3]);
